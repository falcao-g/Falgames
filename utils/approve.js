...
export class Approve extends events {
	...
	async approve() {
		return new Promise(async (resolve) => {
			...
			const btn1 = new ButtonBuilder()
				.setLabel(this.options.buttons.accept)
				.setCustomId("approve_accept")
				.setStyle("SUCCESS")
			const btn2 = new ButtonBuilder()
				.setLabel(this.options.buttons.reject)
				.setCustomId("approve_reject")
				.setStyle("DANGER")
	...
	formatTurnMessage(options, contentMsg) {
		...
		content = content
			.replace("{player.tag}", player1.tag)
