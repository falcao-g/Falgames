...
import { getAlphaEmoji, formatMessage, ButtonBuilder } from "../utils/utils.js"
...
export class Hangman extends events {
	...
	constructor(options = {}) {
		...
		if (typeof options.hangman.shirt !== "string")
			throw new TypeError("INVALID_HANGMAN: hangman shirt must be a string.")
		if (typeof options.hangman.pants !== "string")
			throw new TypeError("INVALID_HANGMAN: hangman pants must be a string.")
		if (typeof options.hangman.boots !== "string")
			throw new TypeError("INVALID_HANGMAN: hangman boots must be a string.")
	...
	foundWord() {
		return this.word
			...
			.replace(/ /g, "")
			.split("")
			.every((l) => this.guessed.includes(l))
	}
	...
	getWordEmojis() {
		return this.word
			.toUpperCase()
			.split("")
			.map((l) => (this.guessed.includes(l) ? getAlphaEmoji(l) : l === " " ? "⬜" : "🔵"))
			.join(" ")
	}
