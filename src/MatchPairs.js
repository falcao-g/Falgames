...
export class MatchPairs extends events {
	...
	constructor(options = {}) {
		...
		if (!options.embed.description)
			options.embed.description = "**Click on the buttons to match emojis with their pairs.**"
		...
		if (!options.loseMessage)
			options.loseMessage = "**You lost the Game! You turned a total of `{tilesTurned}` tiles.**"
		...
		if (options.emojis.length < 12)
			throw new RangeError("INVALID_EMOJIS: Emojis option must contain at least 12 emojis.")
	...
	async handleButtons(msg) {
		...
		collector.on("collect", async (btn) => {
			...
			if (!this.selected) {
				...
				emojiBtn
					.setEmoji(emoji)
					.setStyle(matched ? "SUCCESS" : "DANGER")
					.setDisabled(matched)
					.removeLabel()
				selectedBtn
